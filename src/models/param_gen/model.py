...
# PyTorch >= 2.4 provides nn.RMSNorm; keep a tiny fallback for 2.1–2.3.
if hasattr(nn, "RMSNorm"):
    ...
    class _RMSNorm(nn.Module):
        ...
        def forward(self, x: Tensor) -> Tensor:
            inv = torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)
            x = x * inv
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.norm1 = _RMSNorm(d_model, elementwise_affine=True, eps=1e-6)
        self.norm2 = _RMSNorm(d_model, elementwise_affine=True, eps=1e-6)
        self.norm3 = _RMSNorm(d_model, elementwise_affine=True, eps=1e-6)
        self.norm4 = _RMSNorm(d_model, elementwise_affine=True, eps=1e-6)
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.norm_input = _RMSNorm(self.d_model, elementwise_affine=True, eps=1e-6)
        self.norm_tokens = _RMSNorm(self.d_model, elementwise_affine=True, eps=1e-6)
        ...
        self.norm_final = _RMSNorm(self.d_model, elementwise_affine=True, eps=1e-6)
        ...
        self.use_cond_proj = False
        self.cond_proj = nn.Linear(self.d_model, self.d_model, bias=True)
        nn.init.zeros_(self.cond_proj.weight)
        nn.init.zeros_(self.cond_proj.bias)

        total_tokens = num_base_model_layers * (self.seq_h * self.seq_w // self.dim_accumulation)
        self.use_cond_tokens = False
        self.cond_tokens_mode = "replace"
        self.cond_token_proj = nn.Linear(self.d_model, self.d_model, bias=False)
        self.cond_token_query = nn.Parameter(torch.randn(total_tokens, self.d_model) * 0.002)
        nn.init.zeros_(self.cond_token_proj.weight)

    def init_lora_projection_heads_training(self) -> None:
        nn.init.normal_(self.proj_out_A.weight, mean=0.0, std=0.02)
        if self.proj_out_A.bias is not None:
            nn.init.zeros_(self.proj_out_A.bias)
        nn.init.zeros_(self.proj_out_B.weight)
        if self.proj_out_B.bias is not None:
            nn.init.zeros_(self.proj_out_B.bias)
        if self.use_cond_proj:
            nn.init.zeros_(self.cond_proj.weight)
            nn.init.zeros_(self.cond_proj.bias)
        if self.use_cond_tokens:
            nn.init.zeros_(self.cond_token_proj.weight)
            nn.init.normal_(self.cond_token_query.data, mean=0.0, std=0.002)
    ...
    def _get_raw_pos_tokens(self, batch_size: int) -> Tensor:
        """Return rearranged position embeddings WITHOUT norm_tokens."""
        hw_pos = repeat(self.hw_pos.weight, "hw d -> b n hw d", b=batch_size, n=self.seq_n).clone()
    ...
    def get_tokens(self, batch_size: int) -> Tensor:
        return self.norm_tokens(self._get_raw_pos_tokens(batch_size))
    ...
    def forward(self, encoder_hidden_states: Tensor) -> Tensor:
        # 支持 (B, L, D) 或旧式 (B, 1, L, D)。禁止对 (B, 1, D) 做 squeeze(1)，否则会变成 (B, D) 破坏 cross-attn。
        esh = encoder_hidden_states
        if esh.dim() == 4:
            esh = esh.squeeze(1)
        elif esh.dim() == 2:
            esh = esh.unsqueeze(1)
        if esh.dim() != 3:
            raise ValueError(
                f"encoder_hidden_states 须为 (B, L, D) 或 (B, 1, L, D)，当前 shape={tuple(esh.shape)}"
            )
        encoder_hidden_states = self.norm_input(esh)
        ...
        if self.use_cond_proj:
            cond_global = encoder_hidden_states.mean(dim=1)
            cond_bias = self.cond_proj(cond_global)
            x = x + cond_bias[:, None, None, :]

        if self.use_cond_tokens:
            B = encoder_hidden_states.shape[0]
            cond_kv = self.cond_token_proj(encoder_hidden_states)
            query = self.cond_token_query.unsqueeze(0).expand(B, -1, -1)
            attn_weights = torch.bmm(query, cond_kv.transpose(1, 2))
            attn_weights = attn_weights / (self.d_model ** 0.5)
            attn_weights = torch.softmax(attn_weights, dim=-1)
            cond_tokens = torch.bmm(attn_weights, cond_kv)
            cond_tokens = cond_tokens.view(B, self.seq_n, -1, self.d_model)
            if self.cond_tokens_mode == "replace":
                pos = self._get_raw_pos_tokens(B)
                x = self.norm_tokens(cond_tokens + pos)
            else:
                x = x + cond_tokens
